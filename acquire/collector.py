...
class Collector:
        ...
        for spec_item in spec:
            ...
            for value in values:
                if artifact_type in (ArtifactType.FILE, ArtifactType.DIR):
                    ...
                    self.collect_glob(value, module_name=module_name, skip_mounts=False)
    ...
    def collect_glob(self, pattern: str, module_name: Optional[str] = None, skip_mounts: bool = True) -> None:
        module_name = self.bound_module_name or module_name
        if not module_name:
            raise ValueError("Module name must be provided or Collector needs to be bound to a module")
        ...
        try:
            ...
            for entry in self.target.fs.path("/").glob(pattern.lstrip("/")):
                glob_is_empty = False
                self.collect_path(entry, skip_mounts=skip_mounts)
        except Exception:
